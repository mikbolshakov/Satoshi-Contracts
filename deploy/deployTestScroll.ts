...
// npx hardhat run deploy/deployTestScroll.ts --network scroll_mainnet
async function main() {
  ...
  const Runner2060 = await ethers.getContractFactory('TestScroll');
  const runner2060 = await Runner2060.deploy(maintainer, LZEndpoint, adminAddress);

  await runner2060.deployed();
  console.log(`TestScroll deployed to ${runner2060.address}`);
  ...
  await hre.run('verify:verify', {
    ...
    contract: 'contracts/TestScroll.sol:TestScroll',
  });
}
