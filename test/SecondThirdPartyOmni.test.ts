...
import { Runner2060rewardsOmni, Runner2060rewards, SecondThirdPartyOmni } from '../typechain-types';
...
describe('SecondThirdPartyOmni tests', () => {
  // contracts
  let EndpointV2Mock: ContractFactory;
  let erc1155Linea: Runner2060rewards;
  let thirdPartyLineaOmni: SecondThirdPartyOmni;
  ...
  it('Deploy endpoints and contracts by thirdPartyDeployer', async () => {
    ...
    const FactorySecondThirdPartyOmni = await ethers.getContractFactory('SecondThirdPartyOmni');
    ...
    const omni1155Linea = await FactorySecondThirdPartyOmni.connect(thirdPartyDeployer).deploy(
    ...
    thirdPartyLineaOmni = omni1155Linea as SecondThirdPartyOmni;
  ...
  it('Send tokens from Linea to Scroll (adminLinea => adminScroll)', async () => {
    ...
    await erc1155Linea.connect(adminLinea).burnAdmin(adminLinea.address, zeroTokenId, tenTokens);
  ...
  it('Send tokens from Linea to Scroll (user10 => user20)', async () => {
    ...
    await erc1155Linea.connect(adminLinea).burnAdmin(user10.address, zeroTokenId, tenTokens);
