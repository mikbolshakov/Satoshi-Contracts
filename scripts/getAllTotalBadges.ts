...
const resulPath = 'scripts/ercData/allBalances1155.json';
// const tokenIds = Array.from({ length: 10 }, (_, i) => i); // 1st week
const tokenIds = Array.from({ length: 10 }, (_, i) => i + 10); // 2nd week
...
const main = async () => {
  ...
  fs.writeFileSync(resulPath, JSON.stringify(results, null, 2));
  console.log('Balances have been written to file');
};
